...
func (s *ExportService) Create(ctx context.Context, actor domain.Actor, input CreateExportInput) (domain.ExportResult, error) {
	...
	resultID := s.ids.NewID("export")
	pendingFile := domain.AllocatePendingExport(resultID)
	path, hash, size, err := s.files.Put(ctx, "exports/"+resultID, bytes.NewReader(content))
	if err != nil {
		return domain.ExportResult{}, err
	}
	pendingFile = pendingFile.RecordWrite(path, hash, size)
	...
	sort.Strings(keys)
	result := domain.ExportResult{ID: resultID, RequestID: request.ID, Format: input.Format, Path: pendingFile.StoredName(), SHA256: pendingFile.Digest(), Size: pendingFile.Bytes(), FieldKeys: keys, CreatedAt: s.clock.Now()}
	if err := s.exports.Create(ctx, request, result); err != nil {
		pendingFile = pendingFile.RecordDiscard()
		_ = s.files.Delete(ctx, pendingFile.DiscardName())
		return domain.ExportResult{}, err
	}
	pendingFile = pendingFile.RecordDatabaseCommit()
