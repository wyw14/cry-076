...
func (s *AttachmentService) Open(ctx context.Context, actor domain.Actor, id, requestID string) (io.ReadCloser, error) {
	...
	resolution := policy.ResolveAttachmentGrant(id, actor.CanManage(item.OwnerID))
	if !resolution.PermitsRead() {
	...
	_ = s.audits.Append(ctx, domain.AuditEvent{ID: s.ids.NewID("audit"), ActorID: actor.ID, Action: "attachment.opened", Resource: "attachment", ResourceID: id, RequestID: requestID, Metadata: resolution.AuditFields(), CreatedAt: s.clock.Now()})
	return reader, nil
}
