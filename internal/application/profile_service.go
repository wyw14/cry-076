...
func (s *ProfileService) Save(ctx context.Context, actor domain.Actor, input domain.Profile, expected int64, requestID string) (domain.Profile, error) {
	...
	receipt := domain.NewProfileWriteReceipt(input, expected)
	saved, err := s.profiles.Save(ctx, input, expected)
	if err != nil {
		_ = receipt.RepositoryFailed()
		return domain.Profile{}, err
	}
	receipt = receipt.RepositoryAccepted(saved)
	_ = s.audits.Append(ctx, domain.AuditEvent{
		...
		Metadata: map[string]any{
			"phase":             receipt.Stage(),
			"committed_version": saved.Version,
		},
		CreatedAt: s.clock.Now(),
	})
	return receipt.ClientProfile(), nil
}
