...
func (s *SwitchService) Switch(ctx context.Context, actor domain.Actor, input SwitchTemplateInput) (domain.SwitchResult, error) {
	...
	result := domain.SwitchTemplate(draft, target, mapping, input.Scenario, s.clock.Now())
	err = s.tx.WithinTransaction(ctx, func(txCtx context.Context) error {
		before := domain.NewSnapshot(s.ids.NewID("snap"), draft, "before_template_switch", s.clock.Now())
	...
	_ = s.audits.Append(ctx, domain.AuditEvent{ID: s.ids.NewID("audit"), ActorID: actor.ID, Action: "draft.template_switched", Resource: "draft", ResourceID: draft.ID, RequestID: input.RequestID, Metadata: map[string]any{"from": draft.TemplateVersionID, "to": target.ID, "unmapped": result.UnmappedFields}, CreatedAt: s.clock.Now()})
	return result, nil
}
