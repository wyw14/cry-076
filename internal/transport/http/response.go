...
type transportFailureStage string

const (
	transportFailureMapped     transportFailureStage = "mapped"
	transportFailureCorrelated transportFailureStage = "correlated"
)
...
func mapTransportFailure(err error) transportFailure {
	...
	return transportFailure{stage: transportFailureMapped, status: status, payload: APIError{Code: code, Message: message}}
}

func (f transportFailure) correlate(requestID string) transportFailure {
	f.stage = transportFailureCorrelated
	f.payload.RequestID = requestID
	return f
}

func respondError(c *gin.Context, err error) {
	failure := mapTransportFailure(err).correlate(appmw.GetRequestID(c))
	c.JSON(failure.status, gin.H{"error": failure.payload})
}
