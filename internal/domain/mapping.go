...
type SwitchExecutionStatus string

const (
	SwitchExecutionCreated SwitchExecutionStatus = "created"
	SwitchExecutionSaved   SwitchExecutionStatus = "saved"
)

type SwitchExecution struct {
	status SwitchExecutionStatus
	source Draft
	output SwitchResult
}

func StartSwitchExecution(source Draft, target TemplateVersion, mapping TemplateMapping, scenario TargetScenario, now time.Time) SwitchExecution {
	return SwitchExecution{
		status: SwitchExecutionCreated,
		source: source.Clone(),
		output: SwitchTemplate(source, target, mapping, scenario, now),
	}
}

func (e SwitchExecution) MarkSaved() SwitchExecution {
	if e.status == SwitchExecutionCreated {
		e.status = SwitchExecutionSaved
	}
	return e
}

func (e SwitchExecution) Output() SwitchResult {
	copy := e.output
	copy.Draft = copy.Draft.Clone()
	copy.MappedFields = append([]string(nil), copy.MappedFields...)
	copy.MissingFields = append([]FieldIssue(nil), copy.MissingFields...)
	copy.UnmappedFields = append([]string(nil), copy.UnmappedFields...)
	return copy
}
...
func (e SwitchExecution) CurrentTemplate() string  { return e.output.Draft.TemplateVersionID }
func (e SwitchExecution) Status() SwitchExecutionStatus {
	return e.status
}
